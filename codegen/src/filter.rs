...
impl From<FileFilterTokens> for FileFilter {
  fn from(tokens: FileFilterTokens) -> Self {
    Self {
      items: tokens.items.into_elems().map(FileFilterItem::from).collect(),
    }
  }
}
...
impl Debug for FileFilterItem {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    f.debug_struct("FileFilterItem")
      .field(
        "include",
        &self.includes.iter().map(|item| item.as_str()).collect::<Vec<_>>(),
      )
      .field(
        "exclude",
        &self.excludes.iter().map(|item| item.as_str()).collect::<Vec<_>>(),
      )
      .finish()
  }
}
